...
class Tank_auth
{
	...
	function has_role($role='')
	{
		if($role)
		{
			if($this->ci->session->userdata('role') == $this->get_role_name($role))
			{
				return TRUE;
			}
		}
	...
	function check_username($username){
		if ((strlen($username) > 0) AND $this->is_username_available($username) != 1) {
	...
	function check_email($email){
		if ((strlen($email) > 0) AND $this->is_email_available($email) != 1) {
			$this->error = array('email' => 'auth_email_in_use');
			return 0;
			}
		else return 1;	
	}
	function create_user($username, $email, $password, $email_activation, $nama, $role, $jabatan, $hp, $instansi)
	{
			...
			if (!is_null($res = $this->ci->users->create_user($data, !$email_activation,$nama,$jabatan,$hp,$instansi))) {
